...
const poll = setInterval(() => {
  ...
  if (autoRefresh.value && status.value !== 'pending' && !document.hidden) {
    void refresh()
    void refreshSummary()
  }
}, 30_000)
...
<template>
  <div class="space-y-5">
    ...
    <KpiStrip :summary="summaryData?.data ?? null" :pending="summaryLoading" />
        ...
        >
          ...
          <ul class="divide-y divide-border md:hidden">
            <li v-for="cabinet in rows" :key="cabinet.code">
              <RouterLink :to="`/cabinets/${cabinet.code}`" class="block px-4 py-3.5 hover:bg-surface-2">
